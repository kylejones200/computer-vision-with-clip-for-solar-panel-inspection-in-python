...
def _train_torch(model: nn.Module, X_train, y_train, *, epochs: int=2, batch_size: int=32, lr: float=0.001, validation_split: float=0.2, patience: int=3) -> nn.Module:
...
def prepare_path() -> None:
    ...
    train_ds = utils.image_dataset_from_directory(path, validation_split=0.2, subset='training', image_size=(img_height, img_width), batch_size=32, seed=42)
    val_ds = utils.image_dataset_from_directory(path, validation_split=0.2, subset='validation', image_size=(img_height, img_width), batch_size=32, seed=42)
...
def prepare_x() -> None:
    ...
    early_stopping = callbacks.EarlyStopping(monitor='val_loss', min_delta=0.01, patience=3, verbose=1, restore_best_weights=True)
